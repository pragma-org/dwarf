...
gtip() { ssh dwarf-host-a 'docker exec pb-genesis-victim cardano-cli query tip --testnet-magic 42 --socket-path /state/node.socket 2>/dev/null' 2>/dev/null | python3 -c "import sys,json;print(json.load(sys.stdin).get('block',0))" 2>/dev/null || echo 0; }
atip() { ssh dwarf-host-a 'docker logs pb-amaru-victim 2>&1 | grep "adopted tip" | tail -1' 2>/dev/null | grep -oE "tip.block_height=[0-9]+" | grep -oE "[0-9]+" | tail -1; }
...
for i in $(seq 1 200); do
  ...
  aprev=$A
  [ "$astall" -ge 5 ] && { echo "heal amaru at $A"; ssh dwarf-host-a 'docker restart pb-amaru-victim >/dev/null 2>&1'; astall=0; sleep 15; }
  ...
  if [ "$G" -ge 30500 ] && [ "$A" -ge 30500 ] && [ "$gs" -ge 3 ]; then
    ...
    echo "--- genesis GDD logs ---"; ssh dwarf-host-a 'docker logs pb-genesis-victim 2>&1 | grep -aiE "GDD|density|disconnect|CandidateB" | tail -6'
    exit 0
  fi
  sleep 30
done
