...
|---|---|---|---|---|
| D01 | The live dashboard is on `cardano-box:8787`. | VERIFIED | Container `dwarf-fw` maps `0.0.0.0:8787`; public `/healthz` reports `status: ok`. | `docker inspect dwarf-fw`; `https://dwarf.gainpalfam.com/healthz` |
| D02 | Host is Ryzen 7 6800U, 8 cores / 16 threads, 27 GiB RAM. | VERIFIED | Fresh `lscpu` and `free -h` on `cardano-box`. | host inspection, 2026-09-10 |
...
| D05 | There are 242 scenario definitions. | VERIFIED | Live `/operate` displays 242 and the scenario catalog contains the demo candidate. | `https://dwarf.gainpalfam.com/operate`; `/operate/scenarios` |
...
| D07 | The live dashboard indexes 90 retained run directories. | VERIFIED | Filesystem count is 90; `/metrics` totals 54 pass + 31 fail + 5 error after the rehearsal. This is a current retention-window count, not the lifetime total. | `~/.local/share/dwarf/runs`; `https://dwarf.gainpalfam.com/metrics` |
| D08 | Retained assertions total 65 pass and 32 fail. | VERIFIED | Prometheus counters return those values after the rehearsal. | `https://dwarf.gainpalfam.com/metrics` |
...
| P03 | Run output, assertions, and bundles are retained and inspectable. | VERIFIED | The fallback run page returns 200 and contains manifest, events, assertion, metrics, output, and exact scenario. Eleven unique archives satisfy the current portable-bundle contract; one additional integrity-valid historical archive lacks `manifest.json` and is excluded. | `/operate/runs/20260908T100054Z-80498541`; historical cardano-box bundle census |
