...
def test_block_application_nanosecond_target_identity_is_accepted(tmp_path):
    identity = _identity()
    identity["patch_set_sha256"] = AMARU_BLOCK_APPLICATION_NANOSECOND_PATCH_SHA256
    collector = AmaruPatchedCollector(
        {"id": "amaru-patched-protocol-decode"},
        json_trace_paths=[FIXTURE],
        target_identity=identity,
    ...
    assert result["patch_set_sha256"] == (
        AMARU_BLOCK_APPLICATION_NANOSECOND_PATCH_SHA256
    )
