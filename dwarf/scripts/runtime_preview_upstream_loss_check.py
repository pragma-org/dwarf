...
def _run_ip(args: list[str]) -> None:
    subprocess.run(["sudo", "ip", *args], text=True, capture_output=True, check=False)
...
def _ifb_name_for_implementation(implementation: str) -> str:
    return "ifbdwprla" if implementation == "amaru" else "ifbdwprlh"


def _apply_loss_fault(*, interface: str, ifb_name: str, upstream_ips: list[str], upstream_port: int, loss_pct: int) -> dict:
    existing_qdisc = _tc_qdisc_show(interface)
    added_clsact = "clsact" not in existing_qdisc
    _run_ip(["link", "del", ifb_name])
    _run_ip(["link", "add", ifb_name, "type", "ifb"])
    _run_ip(["link", "set", "dev", ifb_name, "up"])
    if added_clsact:
    ...
    _run_tc(["qdisc", "add", "dev", ifb_name, "root", "netem", "loss", f"{loss_pct}%"])
    return {
        "interface": interface,
        "ifb_name": ifb_name,
...
def _remove_loss_fault(state: dict) -> None:
...
def _emit_window_metrics(
    ...
    *,
    ...
    emit_target_event(
        primitive="runtime_preview_upstream_loss_check",
...
def run_loss(*, scenario_path: Path, fault_seconds: int, recovery_seconds: int, loss_pct: int) -> int:
    ...
    if not active_upstream_ips:
        raise RuntimeError(f"loss fault found no active upstream peer IPs for pid={pid} port={upstream_port}")
    interface_names = sorted({_route_interface_for_ip(ip) for ip in active_upstream_ips})
    if len(interface_names) != 1:
        raise RuntimeError(f"loss fault requires one route interface for active upstream peers, got {interface_names!r}")
    ...
    try:
        ...
        _remove_loss_fault(fault_state)
        fault_active = False
...
def main(argv: list[str]) -> int:
    if len(argv) < 2 or argv[1] != "loss":
        print(
            "usage: runtime_preview_upstream_loss_check.py loss --scenario-path PATH [--fault-seconds N] [--recovery-seconds N] [--loss-pct N]",
            file=sys.stderr,
        )
        return 2
    scenario_path = None
    ...
    while i < len(argv):
        ...
        if argv[i] == "--recovery-seconds" and i + 1 < len(argv):
            ...
            i += 2
            continue
        if argv[i] == "--loss-pct" and i + 1 < len(argv):
            loss_pct = int(argv[i + 1])
            i += 2
            continue
        print(f"unknown argument: {argv[i]}", file=sys.stderr)
        return 2
    if scenario_path is None:
        print("loss mode requires --scenario-path", file=sys.stderr)
        return 2
    return run_loss(
