...
def main(argv=None):
    ...
    p.add_argument("--cargo",type=Path,default=Path.home() / ".cargo/bin/cargo"); p.add_argument("--rustc",type=Path,default=Path.home() / ".cargo/bin/rustc"); p.add_argument("--target-registry",type=Path,default=exact_builder.default_target_registry_root())
    a=p.parse_args(argv)
