...
def _apply_drop_rules(ips: list[str], port: int) -> None:
    for ip in ips:
        _run_iptables(["-I", "OUTPUT", "1", "-p", "tcp", "-d", ip, "--dport", str(port), "-j", "DROP"])


def _remove_drop_rules(ips: list[str], port: int) -> None:
    for ip in ips:
        result = subprocess.run(
            ["sudo", "iptables", "-D", "OUTPUT", "-p", "tcp", "-d", ip, "--dport", str(port), "-j", "DROP"],
            text=True,
            capture_output=True,
            check=False,
        )
...
def _emit_window_metrics(
    ...
    *,
    ...
    if target_implementation == "amaru":
        ...
        emit_runtime_metric(
            f"{prefix}_peer_connection_died_count",
        ...
        emit_runtime_metric(
            f"{prefix}_amaru_peer_connection_died_count",
            value=amaru_trace_metrics["peer_connection_died_count"],
            meta=meta,
        )
    ...
    emit_target_event(
        primitive="runtime_preview_upstream_fault_check",
...
def run_drop(*, scenario_path: Path, fault_seconds: int, recovery_seconds: int) -> int:
    ...
    if not active_upstream_ips:
        raise RuntimeError(f"drop fault found no active upstream peer IPs for pid={pid} port={upstream_port}")
    ...
    _apply_drop_rules(active_upstream_ips, upstream_port)
    fault_active = True
    try:
        ...
        _remove_drop_rules(active_upstream_ips, upstream_port)
        fault_active = False
...
def main(argv: list[str]) -> int:
    if len(argv) < 2 or argv[1] != "drop":
        print(
            "usage: runtime_preview_upstream_fault_check.py drop --scenario-path PATH [--fault-seconds N] [--recovery-seconds N]",
            file=sys.stderr,
        )
        return 2
    scenario_path = None
    ...
    if scenario_path is None:
        print("drop mode requires --scenario-path", file=sys.stderr)
        return 2
    return run_drop(scenario_path=scenario_path, fault_seconds=fault_seconds, recovery_seconds=recovery_seconds)
