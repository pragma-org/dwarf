...
validate term = do
  ...
  case xs of
    [keyTerm, _payload] -> do
      ...
      case key of
        0 -> Right ()
        2 -> termWord16 "failure code" _payload >> Right ()
        3 -> Right ()
        4 -> Right ()
        6 -> Right ()
        _ -> Left ("unexpected localstatequery message key " ++ show key)
    [keyTerm] -> do
      key <- termWord16 "message key" keyTerm
      case key of
        1 -> Right ()
        ...
        9 -> Right ()
        _ -> Left ("unexpected localstatequery message key " ++ show key)
    _ -> Left ("unexpected localstatequery message length " ++ show (length xs))
